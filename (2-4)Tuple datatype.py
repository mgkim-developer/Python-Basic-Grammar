...
# t2 = (1, )처럼 단지 1개의 요소만을 가질 때는 요소 뒤에 콤마(,)를 반드시 분여야 한다는 것
...
# 즉 리스트의 항목 값은 변화가 가능하고 튜플의 항목 값은 변화가 불가능하다. 따라서 프로그램이 실행되는 동안 그 값이 항상 변하지 않기를 바란다거나,
# 값이 바뀔까 걱정하고 싶지 않다면 주저하지 말고 튜플을 사용해야 한다.
# 이와는 반대로 수시로 그 값을 변화시켜야할 경우라면 리스트를 사용해야 한다.
# 실제 프로그램에서는 값이 변경되는 형태의 변수가 훨씬 많기 떄문에 평균적으로 튜플보다는 리스트를 더 많이 사용한다.
...
# t1 = (1, 2, 'a', 'b')
# del t1[0]
#  이것을 실행사면 튜플은 요소를 지우는 행위가 지원되지 않는다는 메시지와 함께 에러가 발생한다.
# 튜플의 요소를 리스트 처럼 del함수로 지우려고 한 예이다. 튜플은 요소를 지우는 행위가 지원되지 않는다.

# 2. 튜플 요솟값을 변경하려 할 때
# t1 = (1, 2, 'a', 'b')
# t1[0] = 'c'
# 위 코드를 실행시켜도 에러가 나는데, 튜플의 요솟값을 변경하려고 해도 마찬가지로 오류가 발생하는 것을 확인할 수 있다.
