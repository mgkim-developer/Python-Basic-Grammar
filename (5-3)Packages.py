...
# ※ echo모듈은 echo.py 파일이다.

# >>> import game.sound.echo
# >>> game.sound.echo.echo_test()
# echo

# 두 번째는 echo 모듈이 있는 디렉토리까지를 from ...import 하여 실행하는 방법이다.

# >>> from game.sound import echo
# >>> echo.echo_test()
# echo

# 세 번쨰는 echo 모듈의 echo_test 함수를 직접 import하여 실행하는 방법이다.
...
# import game을 수행하면 game 디렉토리의 __init__.py에 정의한 것만 참조할 수 있다.
...
# 만약 game, sound, graphic등 패키지에 포함된 디렉토리에 __init__.py 파일이 없다면 패키지로 인식되지 않는다.

# ※ python3.3버전 부터는 __init__.py 파일이 없어도 패키지로 인식한다. 하지만 하위 버전 호환을 위해 __init__.py 파일을 생성하는 것이 안전한 방법이다.
...
# 뭔가 이상하지 않은가? 분명 game.sound 패키지에서 모든 것(*)을 import 하였으므로 echo 모듈을 사용할 수 있어야 할 것같은데, echo라는 이름이 정의되지 않았다는 (NameError)가 발생했다.

# 이렇게 특정 디렉토리의 모듈을 *을 사용하여 import 할 때에는 다음과 같이 해당 디렉토리의 __init__.py 파일에 __all__ 변수를 설정하고 import할 수 있는 모듈을 정의해 주어야 한다.

# # C:/doit/game/sound/__init__.py
# __all__ = ['echo']

# 여기에서 __all__이 의미하는 것은 sound 디렉토리에서 * 기호를 사용하여 import할 경우 이곳에 정의된 echo모듈만 import 된다는 의미이다.
# ※ 착각하기 쉬운데 from.game.sound.echo import* 는 __all__과 상관없이 무조건 import 된다.
# 이렇게 __all__과 상관업싱 무조건 import 되는 경우는 from a.b.c import* 에서 from의 마지막 항목인 c가 모듈인 경우이다.
...
# relative 패키지
# 만약 graphic 디렉토리의 render.py 모듈이 sound 디렉토리의 echo.py 모듈을 사용하고 싶다면 어떻게 해야 할까? 다음과 같이 render.py를 수정하면 가능하다.
...
# 여기에서 ..은 render.py파일의 부모 디렉토리를 의미한다. 따라서 render.py파일의 부모 디렉토리는 game이므로 위와 같은 import가 가능한 것이다.

# ※ render.py 파일의 현재 디렉토리는 graphic이고 부모 디렉토리는 game이다.
