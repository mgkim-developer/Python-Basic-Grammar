...
# 파이썬 2.7 버전까지는 a.keys()함수를 호출할 떄 반환 값으로 dict_keys가 아닌 리스트를 돌려준다.
...
print(a.items())
# itenms 함수는 Key 와 Value의 쌍을 튜플로 묶은 값을 dict_items 객체로 돌려준다.
...
# 다만 다음 예제에서 볼 수 있듯이 a['nokey']처럼 존재하지 않는 키(nokey)로 값을 가져오려고 할 경우 a['nokey']는 Key오류를 발생시키고 a.get('nokey')는 None을 돌려준다는 차이가 있다. 어떤것을 사용할지는 여러분의 선택이다.
a = {'name':'pey', 'phone':'0119993323', 'birth': '1118'}
print(a.get('nokey'))
# print(a['nokey'])
# Traceback (most recent call last):
...
print('name' in a)
print('email' in a)
# 'name' 문자열은 a 딕셔너리의 Key 중 하나이다. 따라서 'name' in a를 호출하면 참(True)를 돌려준다. 반대로 'email'은 a 딕셔너리 안에 존재하지 않는 Key이므로 거짓(False)을 돌려준다.
