...
# a는 2개의 요솟값을 가지고 있기 떄문에 a[3]는 IndexError를 발생시키므로 "인덱싱할 수 없습니다."라는 문자열이 출력될 것이다.
...
# 다음과 같이 ZerroDivisionError와 IndexError를 함꼐 처리할 수도 있다.
...
# 예를 들어 Bird클래스를 상속받는 자식 클래스는 반드시 fly라는 함수를 구현하도록 만들고 싶은 경우(강제로 그렇게 하고 싶은 경우)가 있을 수 있다.
# 다음 예를 보자
...
# 위 예제는 Bird클래스를 상속받는 자식 클래스는 반드시 fly함수를 구현해야 한다는 의지를 보여 준다.
...
# Eagle 클래스는 Bird클래스를 상속받는다. 그런데 Eagle클래스에서 fly함수를 구현하지 않았기 때문에 Bird 클래스의 fly함수가 호출된다.
# 그리고 riase문에 의해 NotImplemented Error가 발생할 것이다.
...
# 그리고 다음과 say_nick("천사")
# say_nick("바보")같이 say_nick 함수를 호출해 보자.
say_nick("천사")
say_nick("바보")
...
# 오류 메시지를 출력했을 때 오류 메시지가 보이게 하려면 오류 클래스에 다음과 같인 __str__ 메서드를 구현해야 한다.
